                                        ...
                                        CUDA::CudaTimerSystem* timer_system)
{
    ...
    TimerSystem::TimeData* exposure_backwards_timer = nullptr;
    TimerSystem::TimeData* wb_backwards_timer       = nullptr;
    TimerSystem::TimeData* vignette_backwards_timer = nullptr;
    TimerSystem::TimeData* response_backwards_timer = nullptr;
    if (timer_system)
    {
        //   exposure_backwards_timer = timer_system->CreateNewTimer("Exposure Backwards");
        //   wb_backwards_timer       = timer_system->CreateNewTimer("WB Backwards");
        //   vignette_backwards_timer = timer_system->CreateNewTimer("Vignette Backwards");
        //   response_backwards_timer = timer_system->CreateNewTimer("Response Backwards");
    }

    std::function<torch::autograd::Variable(torch::autograd::Variable)> timer_grad_for_exp(
        [exposure_backwards_timer](torch::autograd::Variable grad)
        {
            if (exposure_backwards_timer) exposure_backwards_timer->Stop();
            return grad;
        });
    // if (this->is_training()) x.register_hook(timer_grad_for_exp);
    ...
    std::function<torch::autograd::Variable(torch::autograd::Variable)> timer_grad_for_wb(
        [wb_backwards_timer, exposure_backwards_timer](torch::autograd::Variable grad)
        {
            if (exposure_backwards_timer) exposure_backwards_timer->Start();
            if (wb_backwards_timer) wb_backwards_timer->Stop();
            return grad;
        });
    // if (this->is_training()) x.register_hook(timer_grad_for_wb);
    ...
    std::function<torch::autograd::Variable(torch::autograd::Variable)> timer_grad_for_vig(
        [vignette_backwards_timer, wb_backwards_timer](torch::autograd::Variable grad)
        {
            if (wb_backwards_timer) wb_backwards_timer->Start();
            if (vignette_backwards_timer) vignette_backwards_timer->Stop();
            return grad;
        });
    //  if (this->is_training()) x.register_hook(timer_grad_for_vig);
    if (params.enable_vignette)
    {
    ...
    std::function<torch::autograd::Variable(torch::autograd::Variable)> timer_grad_for_res(
        [vignette_backwards_timer, response_backwards_timer](torch::autograd::Variable grad)
        {
            if (vignette_backwards_timer) vignette_backwards_timer->Start();
            if (response_backwards_timer) response_backwards_timer->Stop();
            return grad;
        });
    // if (this->is_training()) x.register_hook(timer_grad_for_res);
    ...
    std::function<torch::autograd::Variable(torch::autograd::Variable)> timer_grad_for_start(
        [vignette_backwards_timer, response_backwards_timer](torch::autograd::Variable grad)
        {
            if (response_backwards_timer) response_backwards_timer->Start();
            return grad;
        });
    // if (this->is_training()) x.register_hook(timer_grad_for_start);
    if (0)
    {
